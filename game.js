...
async function updateLeaderboard(nickname, score) {
    ...
    try {
        ...
        const existingEntry = Object.entries(leaderboard).find(([_, value]) => value.nickname === nickname);
        if (!existingEntry || existingEntry[1].score < score) {
            const newEntry = {
                nickname: nickname,
                score: score,
            ...
            try {
                ...
                if (existingEntry) {
                    await remove(ref(db, `leaderboard/${existingEntry[0]}`));
                }
