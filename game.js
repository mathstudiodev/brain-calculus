...
const sounds = {
    ...
    powerup: new Audio('https://www.soundjay.com/misc/sounds/magic-chime-01.mp3'),
...
async function updateGameStats() {
    try {
        ...
        // Tüm kayıtlardan istatistikleri hesapla
...
async function displayLeaderboard() {
    ...
    try {
        ...
        const noScoresHTML = '<div class="leaderboard-item">Henüz skor yok!</div>';
        
        // Ana sayfa ve oyun sonu leaderboard
        if (leaderboardList) {
            leaderboardList.innerHTML = entries.length > 0 ? leaderboardHTML : noScoresHTML;
        }
        
        // Oyun içi leaderboard
        if (inGameLeaderboard) {
            inGameLeaderboard.innerHTML = entries.length > 0 ? leaderboardHTML : noScoresHTML;
        }
        
        // Mini leaderboard
        if (miniLeaderboard) {
            miniLeaderboard.innerHTML = entries.length > 0 ? leaderboardHTML : noScoresHTML;
        }
        ...
        const errorMessage = '<div class="leaderboard-item">Skor tablosu yüklenirken hata oluştu</div>';
        if (leaderboardList) leaderboardList.innerHTML = errorMessage;
        if (inGameLeaderboard) inGameLeaderboard.innerHTML = errorMessage;
        if (miniLeaderboard) miniLeaderboard.innerHTML = errorMessage;
    }
}
